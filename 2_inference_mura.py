...
from libs.dataset_mura_twoview import create_data_loader, create_test_data_loader
from libs.dataset_mura_twoview import create_data_loader3 as create_data_loader_mura3
...
def parse_option():
    ...
    parser.add_argument('--num_classes', type=int, default=2, help='task number: chexpert:14, mura:2, hipxray:2')
...
def main():
    ...
    for body_part in args.body_parts:
        print(f"\nProcessing body part: {body_part}")
        # test_loader = create_test_data_loader(valid_image_paths_csv, valid_study_labels_csv, body_part, config=config)
        ...
        all_preds = []
        all_labels = []
        for idx, (image1, image2, labels) in tqdm(enumerate(test_loader), total=len(test_loader), desc=f"Body Part: {body_part}"):
            ...
            if 0:
                ...
                visualization = show_cam_on_image(original_bgr, cam_image, use_rgb=True)
                # Plot single overlay with text for each class
                plt.imshow(visualization)
                plt.title(f'GradCAM Visualization', fontsize=16, pad=20)
            
                # Add text for each class prediction
                text_str = ""
                ...
                save_path = Path(output_file).parent / f"epoch_{args.epoch_num}_result"/ f"gradcam_{body_part}" / f"{idx}_gradcam.png"
        ...
        results[body_part] = {'preds': all_preds, 'labels': all_labels}


    plt.figure(figsize=(18, 16))
    scores = []
    for body_part, data in results.items():
        labels = data['labels']
        preds = data['preds']
        roc_auc = roc_auc_score(labels, preds)
        fpr, tpr, _ = roc_curve(labels, preds)
        plt.plot(fpr, tpr, label=f'{body_part} (AUC = {roc_auc:.4f})')
        scores.append((body_part, roc_auc))
    ...
    roc_save_path = roc_save_dir / "all_body_parts_roc_curves.png"

    # Save the ROC curves plot
    plt.tight_layout()
    plt.savefig(str(roc_save_path), bbox_inches='tight')
    plt.close()
    auc_values = [auc_val for _, auc_val in scores if not np.isnan(auc_val)]
    if auc_values:
        average_roc_auc = np.mean(auc_values)
    else:
        average_roc_auc = np.nan
    print(f"\nAverage ROC AUC over 7 body parts: {average_roc_auc:.4f}")
    # roc_auc = roc_auc_score(all_labels, all_preds)
    # # Plot ROC curves
    # roc_plot_path = str(Path(output_file).parent / f"gradcam_{args.body_parts}"/ 'roc_curves.png')
    # plot_roc_curves(all_labels, all_preds, roc_plot_path)
    ...
    roc_scores_path = roc_save_dir / "all_body_parts_roc_scores.txt"
    with open(roc_scores_path, "w") as f:
        f.write("Per-Body-Part ROC AUC Scores:\n")
        for body_part, auc_val in scores:
            f.write(f"{body_part}: {auc_val:.4f}\n")

        f.write(f"\nAverage ROC AUC over {len(scores)} body parts: {average_roc_auc:.4f}\n")
